...
def compute_influence(model, df, X_train, y_train, X_te, y_te):
    sigma_multiplier = 3.0
    infl = LogisticInfluence(model, X_train.values, y_train.values)
    avg_inf = infl.average_influence(X_te.values, y_te.values)
    ...
    print(full_mask.reindex(df.index, fill_value=False).valueses)
    return full_mask.reindex(df.index, fill_value=False).valueses
