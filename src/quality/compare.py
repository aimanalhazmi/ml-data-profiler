...
def compare_outlier_removals(X_train, X_test, y_train, y_test, num_cols, model, alpha = 0.01, sigma_multiplier = 1.0, model_type='logistic'):
    ...
    f1_orig = f1_score(y_test, baseline.predict(X_test.values), average='macro')
    ...
    f1_statistic = f1_score(y_test, stat_clf.predict(X_test.values), average='macro')
    ...
    f1_influence = f1_score(y_test, infl_clf.predict(X_test.values), average='macro')
