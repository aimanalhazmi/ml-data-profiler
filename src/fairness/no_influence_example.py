...
from fairness.no_influence import compute_fairness_metrics
...
X_train, X_test, y_train, y_test, _ , s_test_df, group_train, _= train_test_split(
        X, y, df[sens_cols], df[influence_group_col],
        test_size=0.2, stratify=y, random_state=912
    )
...
no_influence, influence = compute_fairness_metrics(X_train, X_test, y_train, y_test, s_test_df, sens_cols, model, group_train, positive_group)
